class TestWorkflow
  ...
  def initialize(steps=nil, state=nil)
    ...
    @state = state
    initialize_steps(steps || default_steps)
    @execution_path = []
    @log_results = []
  end

  def before_step1; @before_step1_called = true; end
  def after_step1(result); @after_step1_called = true; result; end
  ...
  def sleep_step
    @execution_path << :sleep_step
    Visiflow::Response.sleep(5.minutes.from_now)
  end
