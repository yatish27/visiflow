...
describe Visiflow::Step do
  ...
  describe "self.create_steps" do
    context "when empty" do
      specify { Visiflow::Step.create_steps([]).should == [nil,{}] }
      specify { Visiflow::Step.create_steps(nil).should == [nil,{}] }
    end
    context "when steps exist" do
      ...
      context "and first step is a Symbol" do
        ...
        specify { return_val[0].is_a?(Visiflow::Step).should be_true }
        specify { return_val[0].name === :step1 }
        specify { return_val[1].length.should == 4 }
        specify { return_val[1].class.should == Hash }
      end
